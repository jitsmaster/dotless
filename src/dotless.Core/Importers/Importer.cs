namespace dotless.Core.Importers
{
	...
	public class Importer : IImporter
	{
		...
		protected bool ImportLessFile(string lessPath, Import import)
		{
			...
			else if (IsProtocolUrl(lessPath))
			{
				//AW: use remote reader to parse external urls
				//due to the limitation of ioc container, the override reader has to be done here
				//need to eventually refactor this out
				RemoteReader rr = new RemoteReader();
				contents = rr.GetFileContents(lessPath);
				file = lessPath;
			}
			else
			{
